...
# Hardcoded credentials from your input
...
log = logging.getLogger("SMW-FINAL-BOT")
...
class Bot:
        ...
        try:
            if SESSION_FILE.exists():
                ...
                try:
                    ...
                    log.warning("Saved session expired. Logging in via Username/Password...")
            ...
            log.error("Two-factor authentication required!")
            return False
        except ChallengeRequired:
            log.error("Instagram security checkpoint (Challenge) triggered! Clear it via browser first.")
            return False
        except Exception as e:
    ...
    async def run(self):
        ...
        self.schedule()
        log.info("🔥 BOT IS 100% ONLINE & ARMED | %s", TIMEZONE)
