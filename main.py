...
# Ultra Pro Hardcoded Session ID (Extracted fresh from your screenshot)
SESSION_ID = "27413581604%3A6If3v1JxZKaRwL%3A11%3AAYiXaqK8pbo_69LWZHT2w3iZInLYUZnPfijxJyOg"
...
log = logging.getLogger("SMW-GC-ULTRA-BOT")
...
class Bot:
    ...
    def login(self):
        if not SESSION_ID:
            ...
            return False
        try:
            log.info("Ultra Pro Login: Connecting via Session ID...")
            self.cl.login_by_sessionid(SESSION_ID)
            log.info("Ultra Pro Login Successful! Bot is authenticated.")
            return True
        except Exception as e:
            log.error("Session ID login error: %s", e)
            return False
    ...
    async def run(self):
        ...
        self.schedule()
        log.info("🔥 ULTRA PRO SMW BOT IS 100% ONLINE & ARMED | %s", TIMEZONE)
