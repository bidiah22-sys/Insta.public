...
log = logging.getLogger("SMW-ID-PASS-BOT")
...
class Bot:
    ...
    def login(self):
        if not IG_USERNAME or not IG_PASSWORD or "YAH_APNA" in IG_USERNAME:
            log.error("Please set correct IG_USERNAME and IG_PASSWORD!")
            return False
        
        try:
            # Pehle check karega ki purani saved session file hai kya
            if SESSION_FILE.exists():
                log.info("Loading session from session.json...")
                self.cl.load_settings(SESSION_FILE)
                try:
                    self.cl.get_timeline_feed()
                    log.info("Successfully logged in using saved session file!")
                    return True
                except Exception:
                    log.warning("Saved session expired. Logging in fresh via ID/Password...")

            # Agar session file nahi hai ya expire ho gayi hai, toh direct ID/Password se login karega
            log.info("Logging in with Username: %s", IG_USERNAME)
            self.cl.login(IG_USERNAME, IG_PASSWORD)
            
            # Login hote hi session dump save kar lega taaki dobara password na lage
            self.cl.dump_settings(SESSION_FILE)
            log.info("Login successful and session saved to session.json!")
            return True

        except BadPassword:
            log.error("Wrong password! Please check your credentials.")
            return False
        except TwoFactorRequired:
            log.error("Two-factor authentication required! Please handle 2FA.")
            return False
        except ChallengeRequired:
            log.error("Instagram security checkpoint (Challenge) triggered! Log in via browser first to clear it.")
            return False
        except Exception as e:
    ...
    async def watcher(self):
        log.info("GC Watcher running smoothly | interval=%ss", WATCH_INTERVAL)
    ...
    async def run(self):
        while not self.stop.is_set():
            if self.login():
                break
            log.error("Login attempt failed. Retrying in 30 seconds...")
            await asyncio.sleep(30)

        self.schedule()
        log.info("🔥 ID/PASS SMW BOT IS 100% ONLINE & ARMED | %s", TIMEZONE)
