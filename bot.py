...
# सुरक्षित टाइमर (10 सेकंड) ताकि इंस्टाग्राम बार-बार रिक्वेस्ट भेजने पर आईडी ब्लॉक न करे
...
def start_bot():
    ...
    while True:
        try:
            ...
            # कुकीज़ सेव और लोड करने का लॉजिक - जिससे सेशन कभी न टूटे
            if os.path.exists(SESSION_FILE):
                try:
                    cl.load_settings(SESSION_FILE)
                    cl.login_by_sessionid(SESSION_ID.strip())
                    print("[+] Session re-used successfully from local storage!")
                except Exception:
                    cl.login_by_sessionid(SESSION_ID.strip())
            ...
            while True:
                try:
                    ...
                    print("[!] Session Expired/Blocked! Removing old session and waiting before retry...")
                    if os.path.exists(SESSION_FILE):
