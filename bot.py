                ...
                try:
                    ...
                    for thread in threads:
                        ...
                        if thread_id not in thread_known_members:
                            ...
                            # Check for new members (Welcome / Welcome Back Card)
                            ...
                            # Check for left members (Goodbye Card)
                            ...
                            if message_id and message_id not in seen_message_ids:
                                ...
                                if not is_dev:
                                    ...
                                    if len(user_message_timestamps[sender_id]) > 5:
                                        execute_kick(thread_id, sender_id, sender_username, "Anti-Flood Rapid Spam Triggered", admin_mentions_tag, reply_to_id=message_id)
                                        continue

                                if LOCKDOWN_MODE and not is_dev:
                                    execute_kick(thread_id, sender_id, sender_username, "GC LOCKDOWN ACTIVE", admin_mentions_tag, reply_to_id=message_id)
                                    continue

                                if sender_username.lower() in TARGETED_USERS:
                                    execute_kick(thread_id, sender_id, sender_username, "TARGET ELIMINATED", admin_mentions_tag, reply_to_id=message_id)
                                    continue

                                # --- ULTRA-STRICT AUTO MODERATION (REELS, LINKS, 18+, ABUSE) ---
                                if not is_dev:
                                    is_abuse = is_abusive_text(text)
                                    
                                    # Reel, Link & Post Sharing Detection
                                    ...
                                    if is_abuse or is_reel_or_link or is_nsfw_content:
                                        if is_abuse:
                                            violation_reason = "Abusive / Toxic Language Card Triggered"
                                        elif is_reel_or_link:
                                            violation_reason = "Unauthorized Reel / Link / Promotion Spam Card"
                                        else:
                                            violation_reason = "18+ / NSFW Content / Media / Sticker Card Triggered"
