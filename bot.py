...
def start_bot():
    ...
    while True:
        try:
            print("[*] Connecting to Instagram Engine using Session File...")
            cl = Client()
            ...
            if os.path.exists(SESSION_FILE):
                try:
                    print("[+] Session file found. Loading session directly...")
                    cl.load_settings(SESSION_FILE)
                    # स्ट्रिक्ट सेशन वैलिडेशन चेक
                    cl.get_timeline_feed()
                    print("[+] Successfully logged in strictly via Session File!")
                except Exception as e:
                    print(f"[!] Session invalid or expired ({e}). Please provide a valid session.json file!")
                    time.sleep(30)
                    continue
            else:
                print("[!] ERROR: 'session.json' file not found! Please place your session file in the directory.")
                time.sleep(30)
                continue
            ...
            while True:
                try:
                    ...
                    print("[!] Session Expired! Please update session.json...")
                    time.sleep(30)
                    break
