...
# ==========================================
# LOGGING SETUP (ENTERPRISE GRADE)
# ==========================================
logging.basicConfig(
    level=logging.INFO,
...
logger = logging.getLogger("SMW_ULTRA_GOD_BOT")
...
def get_sender_username(message, thread=None):
    """हरसंभव तरीके से सही यूजरनेम निकालने का अचूक लॉजिक"""
    try:
...
# ==========================================
# MAIN GOD-LEVEL BOT ENGINE
# ==========================================

def start_bot():
    while True:
        try:
            logger.info("Initializing Instagram Client with Full Power...")
            cl = Client()
            try:
                cl.delay_range = [1, 3]
            except Exception:
            ...
            while True:
                try:
                    ...
                    for thread in threads:
                        ...
                        # ==========================================
                        # 🔒 MANDATORY ADMIN CHECK (रूल: एडमिन नहीं तो काम नहीं!)
                        # ==========================================
                        try:
                        ...
                        # अगर बोट इस ग्रुप का एडमिन नहीं है, तो इस ग्रुप को पूरी तरह छोड़ दो!
                        if not is_bot_admin:
                            continue
                        ...
                        if not first_scan:
                            ...
                            for joined_id in newly_joined:
                                ...
                                if welcome_key in recent_welcome_times and (current_time - recent_welcome_times[welcome_key] < 20):
                                    continue
                        ...
                        for message in messages[:6]:
                            ...
                            current_time = time.time()
                            if message_id in recent_action_times and (current_time - recent_action_times[message_id] < 12):
                                processed_messages.add(message_id)
                                continue
                            ...
                            try:
                                # चूंकि बोट खुद एडमिन है, इसलिए sender को चेक करो कि वो एडमिन है या नहीं
                                ...
                                if not is_sender_admin:
                                    ...
                                    # चेक C: गाली / एब्यूसिव वर्ड्स (नॉर्मलाइज्ड मैचिंग)
                                    restricted_words = [
...
if __name__ == "__main__":
    logger.info("Starting Ultra-God-Level Admin-Protected Instagram Bot...")
    start_bot()
