...
def start_bot():
    ...
    while True:
        try:
            ...
            # स्टेप 1: पहले कोड में दी गई Session ID से लॉगिन करने की कोशिश करेगा
            if SESSION_ID and SESSION_ID != "YOUR_SESSION_ID_HERE":
                try:
                    print("[+] Code ke andar di gayi Session ID se login kar rahe hain...")
                    cl.login_by_sessionid(SESSION_ID)
                    logged_in = True
                    print("[+] Session ID se successful login ho gaya!")
                except Exception as e:
                    print(f"[!] Session ID invalid ya expire ho chuki hai: {e}")

            # स्टेप 2: अगर Session ID काम न करे, तो सीधे ID-Password से लॉगिन कर लेगा
            if not logged_in:
                print("[*] Session ID fail ho gayi. Ab direct ID aur Password se login kar rahe hain...")
                try:
                    cl.login(BOT_USERNAME, BOT_PASSWORD)
                    print("[+] ID-Password se successful login ho gaya!")
                except Exception as login_err:
                    print(f"[!] Login Error: {login_err}")
                    print("[!] 30 second baad dobara koshish karenge...")
                    time.sleep(30)
                    continue

            print(f"[+] SUCCESS: Bot @{BOT_USERNAME} FULLY ACTIVE & RUNNING 24x7! 🚀⚡")
            ...
            while True:
                try:
                    ...
                    print("[!] Session expired detected during loop. Re-authenticating...")
                    break
