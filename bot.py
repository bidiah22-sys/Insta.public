...
def start_bot():
    ...
    while True:
        try:
            ...
            # FIXED SAFE SEND MESSAGE FUNCTION (NO EXTRA ARGUMENTS THAT CAUSE CRASH)
                    ...
                    for thread in threads:
                        ...
                        if messages:
                            ...
                            if message_id and message_id not in seen_message_ids:
                                ...
                                sender_username_clean = sender_username.lower()
                                
                                is_sender_admin = sender_id in {str(x) for x in gc_admins}
                                is_sender_dev = sender_username_clean in [dev.lower() for dev in AUTHORIZED_DEVS]
                                ...
                                if text_lower == "!rules":
                                    rules_card = (
                                        ...
                                        f"2️⃣ 🚫 **𝗡𝗢 𝗨𝗡𝗔𝗨𝗧𝗛𝗢𝗥𝗜𝗦🇪🇩 𝗟𝗜𝗡𝗞𝗦** (Spam Restricted)\n"
                                        f"3️⃣ 🚫 **𝗡𝗢 𝗥🇪🇪𝗟𝗦 / 𝗠🇪🇩𝗜𝗔 𝗦𝗣𝗔𝗠𝗠𝗜𝗡𝗚**\n"
                                        f"4️⃣ 👑 **𝗥🇪𝗦𝗣🇪𝗖𝗧 𝗔𝗟𝗟 𝗔𝗗𝗠𝗜𝗡𝗦 & 𝗠🇪𝗠𝗕🇪𝗥𝗦**\n\n"
                                        f"🤖 **𝗠𝗢𝗗🇪𝗥𝗔𝗧🇪🇩 𝗕𝗬** ➜ @{BOT_USERNAME}"
                                    )
                                    ...
                                    safe_send_message(thread_id, f"👋 𝗛🇪𝗟𝗟𝗢 @{sender_username}! 🤖 𝗕𝗢𝗧 𝗜𝗦 𝗔𝗖𝗧𝗜𝗩🇪!")
                                ...
                                if is_sender_dev:
                                    if text_lower == "!health":
                                        ...
                                        safe_send_message(thread_id, SystemMonitor.get_recent_logs())

                                    elif text_lower.startswith("!whois"):
                                        parts = text.split()
                                        if len(parts) > 1:
                                            target_username = parts[1].lstrip('@')
                                            db = SessionLocal()
                                            try:
                                                profile = db.query(UserProfile).filter(UserProfile.username.ilike(target_username)).first()
                                                if profile:
                                                    join_time = profile.join_date.strftime("%Y-%m-%d %H:%M:%S") if profile.join_date else "N/A"
                                                    last_time = profile.last_active.strftime("%Y-%m-%d %H:%M:%S") if profile.last_active else "N/A"
                                                    
                                                    whois_card = (
                                                        f"🕵️‍♂️🔍 **𝗣𝗢𝗢𝗞𝗜🇪🇪 𝗨𝗦🇪𝗥 𝗧𝗥𝗔𝗖𝗞🇪𝗥** 🔍🕵️‍♂️\n\n"
                                                        f"👑 **𝗔𝗖𝗖🇪𝗦𝗦** ➜ DEVELOPERS ONLY\n\n"
                                                        f"🆔 **𝗨𝗦🇪𝗥𝗡𝗔𝗠🇪** ➜ @{profile.username}\n"
                                                        f"🔢 **𝗨𝗦🇪𝗥 𝗜𝗗** ➜ `{profile.user_id}`\n"
                                                        f"💬 **𝗧𝗢𝗧𝗔𝗟 𝗠𝗦𝗚𝗦** ➜ {profile.total_messages}\n"
                                                        f"⚠️ **𝗪𝗔𝗥𝗡𝗜𝗡𝗚𝗦** ➜ {profile.warning_count}/3\n"
                                                        f"🚫 **𝗩𝗜𝗢𝗟𝗔𝗧𝗜𝗢𝗡𝗦** ➜ {profile.violation_count}\n"
                                                        f"📅 **𝗙𝗜𝗥𝗦𝗧 𝗦🇪🇪𝗡** ➜ {join_time} UTC\n"
                                                        f"⏱️ **𝗟𝗔𝗦𝗧 𝗔𝗖𝗧𝗜𝗩🇪** ➜ {last_time} UTC\n\n"
                                                        f"🤖 **𝗕𝗢𝗧** ➜ @{BOT_USERNAME}"
                                                    )
                                                    safe_send_message(thread_id, whois_card)
                                                else:
                                                    safe_send_message(thread_id, f"❌ @{target_username} का रिकॉर्ड बॉट के डेटाबेस में नहीं मिला!")
                                            finally:
                                                db.close()
                                        else:
                                            safe_send_message(thread_id, "⚠️ **Command Syntax:** `!whois @username`")
                                        ...
                                        if len(parts) > 1:
                                            ...
                                            safe_send_message(thread_id, f"✅ **Word Added to Blocklist:** `{new_word}`")
                                        ...
                                        if len(parts) > 1:
                                            ...
                                            safe_send_message(thread_id, f"🗑️ **Word Removed from Blocklist:** `{rem_word}`")
